...
void lcd_set_backlight(uint8_t percent) {
    /* The carrier board has no documented PWM backlight connection.
     * ST7796 commands 0x51/0x53 control optional panel circuitry and make
     * some revisions go completely dark, so brightness stays hardware-set. */
    (void)percent;
}
